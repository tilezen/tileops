...
function filter_basic_tags_rel (keyvalues, numberofkeys)
    ...
    if ((keyvalues["type"] ~= "route") and (keyvalues["type"] ~= "multipolygon") and (keyvalues["type"] ~= "boundary") and (keyvalues["type"] ~= "linestring")) then
        filter = 1
        return filter, keyvalues
    end
    return filter, keyvalues
...
function filter_tags_relation_member (keyvalues, keyvaluemembers, roles, membercount)
    ...
    if (type == "boundary") or (type == "linestring") then
        linestring = 1
    end
